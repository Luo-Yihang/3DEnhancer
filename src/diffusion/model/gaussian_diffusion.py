...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False, loss_weight_mask=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # Changed by Yihang: for Face SR weighted loss:
            if loss_weight_mask is not None:
                weights = th.ones_like(loss)
                weights[loss_weight_mask.expand_as(loss) == 1] = 5
                loss *= weights
